...
async function submit(status) {
  if (!validate()) return
  try {
    await api.put(`/article/${route.params.id}`, { ...form, status })
    router.push('/')
  } catch (e) {
    console.error(e)
  }
...
onMounted(async () => {
  try {
    const post = await api.get(`/article/${route.params.id}`)
...
<template>
  <div class="mx-auto max-w-2xl">
    <h1 class="mb-6 text-2xl font-bold text-slate-800">Edit</h1>
